...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            dists[i, j] = ((x_test_flat[j] - x_train_flat[i]) ** 2).sum()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flat = x_train.flatten(start_dim=1)
    for i in range(num_train):
        dif = x_train_flat[i].unsqueeze(0) - x_test_flat
        dists[i] = (dif ** 2).sum(dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_flat = x_train.flatten(start_dim=1)
    x_test_flat = x_test.flatten(start_dim=1)
    train_sum = (x_train_flat ** 2).sum(dim=1).unsqueeze(1)  # shape (num_train, 1)
    test_sum = (x_test_flat ** 2).sum(dim=1).unsqueeze(0)  # shape (1, num_test)
    cross_term = torch.mm(x_train_flat, x_test_flat.t())
    dists = train_sum + test_sum - 2 * cross_term
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    for j in range(num_test):
        dist_to_test = dists[:, j]
        _, topk_indices = torch.topk(dist_to_test, k=k, largest=False)
        topk_labels = y_train[topk_indices]
        counts = torch.bincount(topk_labels)
        max = counts.max()
        y_pred[j] = torch.where(counts == max)[0].min()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for j in range(num_folds):
        ...
        x_train_fold = torch.cat(x_train_folds[:j] + x_train_folds[j + 1:], dim=0)
        y_train_fold = torch.cat(y_train_folds[:j] + y_train_folds[j + 1:], dim=0)
        model = KnnClassifier(x_train=x_train_fold, y_train=y_train_fold)
        for k in k_choices:
            acc = model.check_accuracy(x_val, y_val, k=k,quiet=True)
